...
class BradleyTerryScaled(BaseModel):
    """
    ...
    .. code-block:: python
        ...
        >>> from leaderbot.algorithms import BradleyTerryScaled

        >>> # Create a model
        >>> data = load_data()
        >>> alg = BradleyTerryScaled(data)
    ...
    def __init__(
        ...
        self.n_weights = 2 * self.n_models
    ...
    def _sample_loss(
        ...
        if return_jac:
            ...
            grads = (grad_xi, grad_xj, grad_ti, grad_tj)
    ...
    def loss(
        ...
        if return_jac:
            grad_xi, grad_xj, grad_ti, grad_tj = grads
            i, j = self.x.T
            n = self.x.shape[0]
            ax = np.arange(n)
