...
@numba.jit(nopython=True)
def davidson_loss(xi, xj, mu, win_ij, loss_ij, tie_ij):
    z = xi - xj
    u = 0.5 * z - mu
    v = -0.5 * z - mu
    p_win = double_sigmoid(z, u)
    ...
    grad_xi = grad_z
    grad_xj = -grad_z
    return loss, grad_xi, grad_xj, grad_mu
...
def davidson_jac(w, x, y, n_models):
    ...
    loss, grad_xi, grad_xj, grad_mu = davidson_loss(xi, xj, mu, win_count, loss_count, tie_count)
