...
@numba.jit(nopython=True)
def raokupper_loss(xi, xj, ti, tj, rij, eta, win_ij, loss_ij, tie_ij):
    s_rij = np.tanh(rij)
    ...
    return loss, grad_xi, grad_xj, grad_ti, grad_tj, grad_rij, grad_eta
...
def raokupper_jac(w, x, y, n_models):
    ...
    loss, grad_xi, grad_xj, grad_ti, grad_tj, grad_rij, grad_eta = raokupper_loss(xi, xj, ti, tj, rij, eta, win_count,
