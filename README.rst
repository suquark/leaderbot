...
`leaderbot` is a python package that provides **leader**\ board for
...
You can check the list of functions, classes, and modules of `leaderbot` with their
usage, options, and examples in `API` references within the documentation.

Quick Usage
===========

In the example below, we use ``leaderbot.DavidsonSaled`` class to build a
model. Users can choose from many other models provided in the `API` reference.
...
``leaderbot.BaseModel.rank`` function:
...
The above code provides the text below:
...
The above code also produces the following plot:
...
The results can be visualized with ``leaderboard.BaseModel.visualize``
...
loss, or tie for a pair of agents using :``leaderbot.BaseModel.infer``
method:
