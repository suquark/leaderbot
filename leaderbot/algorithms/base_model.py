...
class BaseModel(object):
    ...
    infer
        Makes inference of game probabilities based on a trained model weights.

    predict
        Predicts output of a game between players.

    rank
        Provide leader-board table and plot for the models.
    ...
    def _cumulative_counts(
            ...
            density: bool = False):
        """
        Cumulative number of wins, losses, and ties for each player with
        respect to all its plays against all other players.
        ...
        x : array_like, default=None
            An array of size (n_pairs, 2) with rows ``(i, j)`` representing the
            game between the ``i``-th and ``j``=th player. If ``None``, the
            variable ``x`` from the input data is used.

        y : array_like, default=None
            An array of size (n_pairs, 3) with columns representing wins,
            losses, and ties. If `None`, the variable ``y`` from the input data
            is used.
        ...
        n_wins = np.zeros((n_models,), dtype=int)
        n_losses = np.zeros((n_models,), dtype=int)
        n_ties = np.zeros((n_models,), dtype=int)
        n_games = np.zeros((n_models,), dtype=int)

        for i in range(n_models):

            # Find all games with the i-th player
            ind_0 = (x[:, 0] == i)
            ind_1 = (x[:, 1] == i)
        ...
        if density:
            # Normalize by the total counts of each player
    ...
    def _covariance(
        ...
        if self.weights.size < 2 * self.n_models:
            # The algorithm does not have Thurstonian model, so no covariance
            return None

        # Diagonals of covariance matrix
        ti = np.abs(self.weights[self.n_models:2*self.n_models])

        # Off-diagonals of correlation matrix
        n_pairs = self.n_models * (self.n_models - 1) // 2
        if self.weights.size >= 2*self.n_models + n_pairs:
            rij = self.weights[2*self.n_models:2*self.n_models + n_pairs]
        else:
            rij = None

        # S is Covariance
        S = np.zeros((self.n_models, self.n_models), dtype=float)

        for j in range(self.n_models):
            for i in range(j, self.n_models):
        ...
        if centered:
            ...
            v = np.ones((self.n_models, 1))
            C = np.eye(self.n_models) - v @ v.T / self.n_models
    ...
    def _distance_matrix(self):
        """
        Distance matrix between mutual models.
        """
        ...
        # Scores
        xi = self.weights[:self.n_models]

        # Distance matrix
        D = np.zeros((self.n_models, self.n_models), dtype=float)
    ...
    def train(
        ...
        init_weight : array_like, default=None
            Initial weight parameters. If `None`, an initial guess is used
            based on the cumulative counts between model games.
        ...
        .. code-block:: python
            ...
            >>> from leaderbot.algorithms import DavidsonScaled

            >>> # Create a model
            >>> data = load_data()
            >>> alg = DavidsonScaled(data)
        ...
        # Check or initialize weights
        if init_weights is None:
            init_weights = self._initialize_weights()
    ...
    def infer(
        ...
        _, _, probs = self._sample_loss(self.weights, x, None, self.n_models,
    ...
    def predict(
            self,
            x: Union[List[float], np.ndarray[np.floating]] = None):
        """
        Predict win, loss, or tie between players.

        Parameters
        ----------

        x : list or array_like, default=None
            A list of pairs ``(i, j)``, or a numpy array of shape
            ``(n_players, 2)`` where each row is the indices ``(i, j)`` of the
            players. If `None`, the ``X`` variable from the input data is used.
        ...
        prob : np.array
            An array of the shape ``(n_players, )`` where each entry represents
            the following codes:

            * ``+1``: player ``i`` wins ``j``.
            * ``0``: player ``i`` ties ``j``.
            * ``-1``: player ``i`` losses to ``j``.
        ...
        probs = self.infer(x)
        max_ind = np.argsort(probs, axis=1)
    ...
    def rank(
        ...
        max_rank : int, default=None
            The maximum number of players to be displayed. If `None`, all
            players in the input dataset will be ranked and shown.

        plot : bool, default=False
            If `True`, the observed and predicted frequencies of games will be
        ...
        if max_rank is None:
            max_rank = self.n_models
        elif max_rank > self.n_models:
        ...
        # Scores are the x_i, x_j parameters across all algorithms
        score = self.weights[:self.n_models]
        ...
        print('|                           |        |    num |   observed   ' +
              ' |   predicted   |')
        print('| rnk  model                |  score |  games | win loss  tie' +
              ' | win loss  tie |')
        print('+---------------------------+--------+--------+--------------' +
              '-+---------------+')
    ...
    def visualize(
        ...
        models_ranked = np.array(self.models)[rank_]
        ...
        # Visualize
        with texplot.theme(rc={'font.family': 'san-serif'}):
            ...
            if dim == '2d':
                ...
                for i, name in enumerate(models_ranked[:]):
