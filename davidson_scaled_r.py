...
@numba.jit(nopython=True)
def davidson_loss(xi, xj, ti, tj, r, mu, win_ij, loss_ij, tie_ij):
    s_r = np.tanh(r)
    ...
    return loss, grad_xi, grad_xj, grad_ti, grad_tj, grad_r, grad_mu
...
def davidson_jac(w, x, y, n_models):
    ...
    loss, grad_xi, grad_xj, grad_ti, grad_tj, grad_r, grad_mu = davidson_loss(xi, xj, ti, tj, r, mu, win_count,
    ...
    # print(f"Loss: {loss}. Constraint: {constraint_diff}. Nu: {np.exp(mu)}, R: {r}")
    return loss, jac
